...
from model_GAN import AdaGAN_GEN
...
class Evaluator(object):
    ...
    def build_model(self): 
        if self.args.model_type == 'AdaGAN':
            self.model = cc(AdaGAN_GEN(self.config))
        else:
            print('No model')
    ...
    def load_model(self):
        if self.args.model_type == 'AdaGAN':
            model_path = os.path.join('./model_AdaGAN', 'model-{}.ckpt'.format(self.args.load_iteration))
        print('Load model from {}'.format(model_path))
        self.model.load_state_dict(torch.load(model_path)['G_model'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', default='AdaGAN', help='[AdaVAE, AdaVAEd, AdaVAEavg]')
