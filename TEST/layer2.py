...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self._mode == 'attn-vqvae':
            ...
            # encoding_indices = torch.randint(low=0, high=self._num_embeddings, size=encoding_indices.size()).cuda()
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, d_vae=16, dropout=0.1):
        super().__init__()
        ...
        self.mean = nn.Linear(d_v, d_vae)
        self.std = nn.Linear(d_v, d_vae)
        self.transform = nn.Linear(d_vae,d_v)

    def attention_result(self, q, k, v, evaluate=False):
        ...
        if evaluate:
            # mu = torch.randn(mu.size(0), mu.size(1), mu.size(2)).cuda()
            v_cont = mu
            loss_kl = None
        else:
            log_sigma = self.std(v[:,:,0,:])
            eps = log_sigma.new(*log_sigma.size()).normal_(0, 1)
            v_cont = mu + torch.exp(log_sigma / 2) * eps
            loss_kl = 0.5 * torch.mean(torch.exp(log_sigma) + mu ** 2 - 1 - log_sigma)

        v_cont = self.transform(v_cont)
        loss_vq, v_disc, perplexity, indices = self._vq_vae(v[:,:,1,:])

        v = torch.cat((v_cont.unsqueeze(2),v_disc.unsqueeze(2)),dim=2)
        ...
        return output, loss_kl, loss_vq, perplexity, indices, attn

    def forward(self, q, k, v):
        sz_b, len_q, _ = q.size()
        output, loss_kl, loss_vq, perplexity, indices, attn = self.attention_result(q,k,v)
        ...
        return output, attn, loss_kl, loss_vq, perplexity, indices

    def inference(self,x, x_p):
        sz_b, len_q, _ = x.size()
        output, loss_kl, loss_vq, perplexity, indices, attn = self.attention_result(x,x,x,evaluate=True)
        output_p, _,_,_,_,_ = self.attention_result(x_p,x_p,x_p,evaluate=True)
        # n, b, lq, dv # [c,d]
        # output[0,:,:,:] = output_p[0,:,:,:]
