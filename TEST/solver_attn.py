...
class Solver_attn(object):
    ...
    def ae_step(self, data, data_d, data_d_len, lambda_kl):
        ...
        x_d = cc(data_d)
        x_d_len = data_d_len
        loss_kl, loss_vq, emb, dec, perplexity, indices = self.model(x, x_d, x_d_len)
        criterion = nn.L1Loss()
        loss_rec = criterion(dec, x)
        loss = self.config['lambda']['lambda_rec'] * loss_rec + loss_vq + lambda_kl * loss_kl
        self.opt.zero_grad()
        loss.backward()
    ...
    def train(self, n_iterations):
        for iteration in range(n_iterations):
            ...
            if iteration % self.args.summary_steps == 0:
                self.logger.scalars_summary(f'{self.args.tag}/vqvae_attn', meta, iteration)
            ...
            print(f'VQVAE:[{iteration + 1}/{n_iterations}], loss_rec={loss_rec:.3f}, loss_kl={loss_kl:.3f}, '
                    f'loss_vq={loss_vq:.5f}, perplexity={perplexity:.3f}, select={select}/{all_select}     ', end='\r')
