...
from layer2 import VectorQuantizer, MultiHeadAttention
...
class VQVAE_attn(nn.Module):
    def __init__(self, config, speaker_encoder):
        super(VQVAE_attn, self).__init__()
        self.speaker_encoder = speaker_encoder
        self.content_encoder = ContentEncoder(**config['ContentEncoder'])
        self.decoder = Decoder(**config['Decoder'])
        self.self_attention = MultiHeadAttention(n_head=2, d_model=128, d_k=64, d_v=64)

    def forward(self, x, x_s, x_s_len):
        s_emb = self.get_speaker_embeddings(x_s, x_s_len)
        c_emb = self.content_encoder(x)
        c_emb = c_emb.permute(0,2,1)
        c_emb, attn, loss_kl, loss_vq, perplexity, indices = self.self_attention(c_emb,c_emb,c_emb)
        c_emb = c_emb.permute(0,2,1)
        dec = self.decoder(c_emb, s_emb)

        return loss_kl, loss_vq, s_emb, dec, perplexity, indices

    def inference(self, x, x_p, x_s, x_s_len):
        s_emb = self.get_speaker_embeddings(x_s, x_s_len)
        c_emb = self.content_encoder(x).permute(0,2,1)
        c_emb_p = self.content_encoder(x_p).permute(0,2,1)
        c_emb, attn, loss_kl, loss_vq, perplexity, indices = self.self_attention.inference(c_emb,c_emb_p)
        c_emb = c_emb.permute(0,2,1)
        ...
        #     plt.imshow(attn[i,:,:].cpu().detach().numpy(), aspect="auto", origin="lower", interpolation=None)
    ...
    def get_vqvae_embeddings(self):
        return self.self_attention._vq_vae._embedding.weight.data.cpu()
