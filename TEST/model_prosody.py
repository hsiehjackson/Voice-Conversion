...
def append_cond(x, cond):
    # x = [batch_size, x_channels, 128/64/32/16]
    # cond = [batch_size, x_channels, 16]
    # p = cond.size(1) // 2
    # mean, std = cond[:, :p], cond[:, p:]
    # out = x * std.unsqueeze(dim=2) + mean.unsqueeze(dim=2)
    cond = cond.unsqueeze(3).repeat(1,1,1,ceil(x.size(2)/cond.size(2))).view(cond.size(0),cond.size(1),-1)
    cond = cond[:,:, :x.size(-1)]
    out = x + cond
    return out
...
class ProsodyEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # avg pooling
        mean = self.pooling_layer(out).expand(-1, -1, out.size(-1))
        out = out - mean
        out = out.permute(0,2,1) # 128, 16, 128
        # dense blocks
...
class Decoder(nn.Module):
    def __init__(self, 
        ...
        self.second_conv_layers = nn.ModuleList(\
                [f(nn.Conv1d(c_h * 2, c_h * up, kernel_size=kernel_size)) \
                for _, up in zip(range(n_conv_blocks), self.upsample)])

        self.norm_layer = nn.InstanceNorm1d(c_h, affine=False)
        
        self.conv_affine_layers_pd = nn.ModuleList(
                [f(nn.Linear(4, c_h)) for _ in range(n_conv_blocks*2)])

        self.conv_affine_layers_sp = nn.ModuleList(
                [f(nn.Linear(c_cond, c_h)) for _ in range(n_conv_blocks*2)])

        self.out_conv_layer = f(nn.Conv1d(c_h, c_out, kernel_size=1))
        self.dropout_layer = nn.Dropout(p=dropout_rate)

    def forward(self, z, sp_cond, pd_cond):
        ...
        for l in range(self.n_conv_blocks):
            y = pad_layer(out, self.first_conv_layers[l])
            y = self.norm_layer(y)
            y = append_cond(y, self.conv_affine_layers_pd[l*2](pd_cond).permute(0,2,1))
            # 128 128 16
            y = concat_cond(y, self.conv_affine_layers_sp[l*2](sp_cond))
            # 128 256 16
            y = self.act(y)
            y = self.dropout_layer(y)
            y = pad_layer(y, self.second_conv_layers[l])
            ...
            # 128 128 32
            y = self.norm_layer(y)
            # y = append_cond(y, self.conv_affine_layers_pd[l*2+1](pd_cond))
...
class AE_D_Prosody(nn.Module):
    def __init__(self, config, speaker_encoder):
        super(AE_D_Prosody, self).__init__()
        self.speaker_encoder = speaker_encoder
        self.prosody_encoder = ProsodyEncoder(**config['SpeakerEncoder']) 
        self.content_encoder = ContentEncoder(**config['ContentEncoder'])
        self.decoder = Decoder(**config['Decoder'])

    def forward(self, x, x_d, x_d_len):
        sp_emb = self.get_speaker_embeddings(x_d, x_d_len)
        pd_emb = self.prosody_encoder(x)
        mu, log_sigma = self.content_encoder(x)
        # x 128, 80, 128
        # mu 128, 128, 16
        eps = log_sigma.new(*log_sigma.size()).normal_(0, 1)
        dec = self.decoder(mu + torch.exp(log_sigma / 2) * eps, sp_emb, pd_emb)
        return mu, log_sigma, sp_emb, pd_emb, dec

    def inference(self, x, x_p, x_d, x_d_len):
        sp_emb = self.get_speaker_embeddings(x_d, x_d_len)
        pd_emb = self.prosody_encoder(x_p)
        mu, _ = self.content_encoder(x)
        #emb = torch.zeros_like(emb).cuda()
        dec = self.decoder(mu, sp_emb, pd_emb)
        return dec
