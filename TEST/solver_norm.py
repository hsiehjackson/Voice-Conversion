...
class Solver_norm(object):
    ...
    def inference_one_utterance(self, x, xt_d, xt_d_len):
    ...
    def ae_step(self, data, data_d, data_d_len):
        ...
        emb, dec = self.model(x, x_d, x_d_len)
        #emb, dec = self.model(x[0].unsqueeze(0), x_d[0].unsqueeze(0), [x_d_len[0]])
        ...
        for i in range(len(data)): 
            ...
            source_spec_rec = self.inference_one_utterance(source_spec, source_dspec, data_d_len[i])
            plot_spectrograms(source_spec.cpu().numpy(), 'src_sync.png')
            plot_spectrograms(source_spec_rec.cpu().numpy(), 'src_rec_one.png')
            plot_spectrograms(dec[i].detach().cpu().permute(1,0).numpy(), 'src_rec_batch.png')
            criterion = nn.L1Loss()
            loss_src_rec = criterion(source_spec_rec.cpu(), source_spec.cpu())
            loss_src_rec2 = criterion(dec[i].detach().cpu().permute(1,0), source_spec.cpu())
            print('Source one Loss: {} | batch Loss: {}'.format(loss_src_rec, loss_src_rec2))
            input()
        '''
        return meta
    ...
    def train(self, n_iterations):
        for iteration in range(n_iterations):
            ...
            if iteration % self.args.summary_steps == 0:
                self.logger.scalars_summary(f'{self.args.tag}/pd_norm', meta, iteration)

            loss_rec = meta['loss_rec']
            print(f'Norm:[{iteration + 1}/{n_iterations}], loss_rec={loss_rec:.3f}', end='\r')
