...
class GST(nn.Module):
    def __init__(self, config, speaker_encoder):
        ...
        self.stl = STL(token_num=4, num_heads=4, E=128)
    ...
    def forward(self, x, x_s, x_s_len):
        ...
        c_emb = self.stl(c_emb)
        c_emb = c_emb.permute(0,2,1)
        dec = self.decoder(c_emb, s_emb)

        return loss, s_emb, dec, perplexity, indices

    def inference(self, x, x_s, x_s_len):
        s_emb = self.get_speaker_embeddings(x_s, x_s_len)
        c_emb = self.content_encoder(x)
        loss, c_emb, perplexity, indices = self._vq_vae(c_emb)
        c_emb = c_emb.permute(0,2,1)
        c_emb, attn = self.self_attention(c_emb,c_emb,c_emb)
        c_emb = c_emb.permute(0,2,1)
