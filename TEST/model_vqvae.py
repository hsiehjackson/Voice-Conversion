...
from layer import VectorQuantizer, MultiHeadAttention
...
class VQVAE(nn.Module):
    def __init__(self, config, speaker_encoder):
        ...
        self.self_attention = MultiHeadAttention(n_head=4, d_model=128, d_k=32, d_v=32)
    ...
    def forward(self, x, x_s, x_s_len):
        ...
        c_emb, attn, loss, perplexity, indices = self.self_attention(c_emb,c_emb,c_emb)
        c_emb = c_emb.permute(0,2,1)
        dec = self.decoder(c_emb, s_emb)

        return loss, s_emb, dec, perplexity, indices
    ...
    def inference(self, x, x_p, x_s, x_s_len):
        ...
        c_emb, attn, loss, perplexity, indices = self.self_attention.inference(c_emb,c_emb_p)
        c_emb = c_emb.permute(0,2,1)
