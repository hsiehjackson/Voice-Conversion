...
class AdaAE(nn.Module):
    def __init__(self, config):
        super(AdaAE, self).__init__()
    ...
    def forward(self, x):
        style = self.speaker_encoder(x)
        content = self.content_encoder(x)
        dec = self.decoder(content, style)
        return content, style, dec

    def inference(self, x, x_d):
        style = self.speaker_encoder(x_d)
        content = self.content_encoder(x) # N, E, T
        dec = self.decoder(content, style)
        return dec
