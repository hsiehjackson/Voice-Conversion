...
class AE(nn.Module):
    ...
    def inference(self, x, x_cond, x_d_len):
...
class SpeakerEncoder(nn.Module):
    def __init__(self, resembler):
        super(SpeakerEncoder, self).__init__()
        self.resembler = resembler
    def forward(self, x, length):
        emb = []
        for d, l in zip(x, length):
            emb.append(d[:l])
        emb = torch.cat(emb,dim=0)   
        emb = self.resembler.embed_dvector_torch(emb, length)
        return emb

class AdaVAEd(nn.Module):
    def __init__(self, config, resembler):
        super(AdaVAEd, self).__init__()
        self.speaker_encoder = SpeakerEncoder(resembler)
        self.content_encoder = ContentEncoder(**config['ContentEncoder'])
        self.decoder = Decoder(**config['Decoder'])

    def forward(self, x, x_d, x_d_len):
        emb = self.speaker_encoder(x_d, x_d_len)
        mu, log_sigma = self.content_encoder(x)
        eps = log_sigma.new(*log_sigma.size()).normal_(0, 1)
        dec = self.decoder(mu + torch.exp(log_sigma / 2) * eps, emb)
        return mu, log_sigma, emb, dec
    ...
    def inference(self, x, x_d):
        emb = self.speaker_encoder(x_d, [len(x_d.squeeze())])
        mu, _ = self.content_encoder(x) # N, E, T
        dec = self.decoder(mu, emb)
        return dec

    def get_speaker_embeddings(self, x, x_d_len):
        emb = self.speaker_encoder(x, x_d_len)
        return emb