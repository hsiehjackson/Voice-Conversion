...
from classify_dataset import ClassifyTestDateset, ClassifyTestDateset_d
from model_AE import AdaAE
from model_VAE import AdaVAE
from model_VAEVAE import AdaVAEVAE
...
class Evaluator(object):
    ...
    def load_model(self):
        if self.args.model_type == 'AdaVAEd':
            model_path = os.path.join('./model_AdaVAEd_train', 'model-{}.ckpt'.format(self.args.load_iteration))
            ...
            model_path = os.path.join('./model_AdaAE', 'model-{}.ckpt'.format(self.args.load_iteration))
        elif self.args.model_type == 'AdaVAEVAE':
            model_path = os.path.join('./model_AdaVAEVAE', 'model-{}.ckpt'.format(self.args.load_iteration))
        elif self.args.model_type == 'AdaVAEaug':
            model_path = os.path.join('./model_AdaVAE_aug', 'model-{}.ckpt'.format(self.args.load_iteration))
        ...
        if self.args.infer_sproofing:
            self.Speakerclassifier.load_state_dict(torch.load("/home/mlpjb04/Voice_Conversion/hsieh_code_VAE/model_Classifier_512/model-140000-99.96.ckpt")["model"])
    ...
    def plot_transfer_embeddings(self):
        ...
        for data in dataloader:
            ...
            use_spec = spec_tar_d if self.args.model_type == 'AdaVAEd' else spec_tar
            emb = self.model.get_speaker_embeddings(use_spec)

            with torch.no_grad():
                spec_tran = self.model.inference(spec_src, use_spec)
    ...
    def infer_sproofing(self,):
        ...
        if args.model_type == 'AdaVAEd':
            dataset = ClassifyTestDateset_d(os.path.join(self.args.data_dir, self.args.dataset), speakers, utts, self.indexes, self.speaker2id, segment_size=None)
        else:
        ...
        for data in tqdm(dataloader):
            ...
            with torch.no_grad():
                use_spec = spec_tar_d if self.args.model_type == 'AdaVAEd' else spec_tar
                spec_conv = self.model.inference(spec_src, use_spec)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', default='AdaVAE', help='[AdaVAE, AdaVAEd, AdaAE]')
