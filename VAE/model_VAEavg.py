...
class AdaVAEavg(nn.Module):
    def __init__(self, config):
        super(AdaVAEavg, self).__init__()
    ...
    def train_average(self, x1, x2):
        # Encode
        spe1 = self.speaker_encoder(x1)
        mu_1, log_sigma_1 = self.content_encoder(x1)

        spe2 = self.speaker_encoder(x2)
        mu_2, log_sigma_2 = self.content_encoder(x2)

        # Average
        spe = (spe1+spe2)/2
        eps_1 = log_sigma_1.new(*log_sigma_1.size()).normal_(0, 1)
        eps_2 = log_sigma_2.new(*log_sigma_2.size()).normal_(0, 1)

        # Decode
        dec1 = self.decoder(mu_1 + torch.exp(log_sigma_1 / 2) * eps_1, spe)
        dec2 = self.decoder(mu_2 + torch.exp(log_sigma_2 / 2) * eps_2, spe)
