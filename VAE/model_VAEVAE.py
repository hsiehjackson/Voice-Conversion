...
class AdaVAEVAE(nn.Module):
    def __init__(self, config):
        super(AdaVAEVAE, self).__init__()
    ...
    def forward(self, x):
        mu_s, log_sigma_s = self.speaker_encoder(x)
        eps_s = log_sigma_s.new(*log_sigma_s.size()).normal_(0, 1)

        mu_c, log_sigma_c = self.content_encoder(x)
        eps_c = log_sigma_c.new(*log_sigma_c.size()).normal_(0, 1)
        
        content = mu_c + torch.exp(log_sigma_c / 2) * eps_c
        style = mu_s + torch.exp(log_sigma_s / 2) * eps_s

        dec = self.decoder(content, style)
        return mu_s, log_sigma_s, mu_c, log_sigma_c, dec

    def inference(self, x, x_d):
        mu_s, _ = self.speaker_encoder(x_d)
        mu_c, _ = self.content_encoder(x) # N, E, T
        dec = self.decoder(mu_c, mu_s)
        return dec

    def get_speaker_embeddings(self, x):
        emb, _ = self.speaker_encoder(x)
        return emb
